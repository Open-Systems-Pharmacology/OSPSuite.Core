      ...
      protected override void Context()
      {
         ...
         A.CallTo(() => _dataSourceLLOQ.ImportedDataSetAt(A<int>.Ignored)).Returns(new ImportedDataSet
            (
               "file",
               "sheet1",
               new ParsedDataSet(new List<(string, IReadOnlyList<string>)>(), A.Fake<IUnformattedData>(), new List<UnformattedRow>(), _parsedDataSetLLOQ),
               "name",
         ...
         A.CallTo(() => _dataSourceInconsistentLLOQ.ImportedDataSetAt(A<int>.Ignored)).Returns(new ImportedDataSet
            (
               "file",
               "sheet1",
               new ParsedDataSet(new List<(string, IReadOnlyList<string>)>(), A.Fake<IUnformattedData>(), new List<UnformattedRow>(), _parsedDataSetInconsistentLLOQ),
               "name",
               new List<MetaDataInstance>()
            )
         );
         ...
         A.CallTo(() => _dataSourceUnitFromColumn.ImportedDataSetAt(A<int>.Ignored)).Returns(new ImportedDataSet
            (
               "file",
               "sheet1",
               new ParsedDataSet(new List<(string, IReadOnlyList<string>)>(), A.Fake<IUnformattedData>(), new List<UnformattedRow>(), _parsedDataSetUnitFromColumn),
               "name",
               new List<MetaDataInstance>()
            )
         );
