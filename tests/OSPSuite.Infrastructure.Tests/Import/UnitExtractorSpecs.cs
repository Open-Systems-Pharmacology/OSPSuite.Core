...
namespace OSPSuite.Infrastructure.Import
{
   ...
   public class When_testing_different_combination_of_text : concern_for_UnitExtractor
   {
      ...
      [TestCase("[Value] [unit]", "[Value]", "unit")]
      public void should_return_the_expected_value(string text, string label, string unit)
      {
         var res = UnitExtractor.ExtractLabelAndUnit(text);
         res.label.ShouldBeEqualTo(label);
