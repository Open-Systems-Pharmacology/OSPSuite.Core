...
namespace OSPSuite.Infrastructure.Import
{
   public abstract class concern_for_DataSource : ContextSpecification<IDataSource>
   {
      ...
      protected override void Context()
      {
         ...
         _fakeDataSet.AddData(new List<ParsedDataSet>() {new ParsedDataSet(new List<string>(), A.Fake<IDataSheet>(), new List<UnformattedRow>(),
            parsedData)});
   ...
   public class When_validating_empty_data_source : concern_for_DataSource
   {
      ...
      protected override void Context()
      {
         ...
         dataSet.AddData(new List<ParsedDataSet>() { { new ParsedDataSet(new List<string>(), A.Fake<IDataSheet>(), new List<UnformattedRow>(), parsedData) } });
   ...
   public class When_validating_geometric_error : concern_for_DataSource
   {
      ...
      protected override void Context()
      {
         ...
         _dataSet.AddData(new List<ParsedDataSet>() { { new ParsedDataSet(new List<string>(), A.Fake<IDataSheet>(), new List<UnformattedRow>(), parsedData) } });
         sut.DataSets.Clear();
