...
namespace OSPSuite.Presentation
{
   public abstract class concern_for_WeightedDataRepositoryToDataTableMapper : ContextSpecification<WeightedDataRepositoryToDataTableMapper>
   {
      ...
      protected override void Context()
      {
         ...
         A.CallTo(() => _dataRepositoryTask.ToDataTable(A<IEnumerable<DataColumn>>._, A<bool>._)).Returns(new[] {new DataTable() });
      }
   }
   ...
   public class When_mapping_a_weighted_data_repository : concern_for_WeightedDataRepositoryToDataTableMapper
   {
      ...
      {
         A.CallTo(() => _dataRepositoryTask.ToDataTable(A<IEnumerable<DataColumn>>.That.Matches(repo => repo.Count() == 3), A<bool>._)).MustHaveHappened();
      }

      [Observation]
      public void should_convert_the_observed_data_to_a_data_table()
      {
         A.CallTo(() => _dataRepositoryTask.ToDataTable(A<IEnumerable<DataColumn>>.Ignored, false)).MustHaveHappened();
      }
   }
}
