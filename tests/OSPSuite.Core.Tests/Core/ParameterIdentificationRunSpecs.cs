...
namespace OSPSuite.Core
{
   public abstract class concern_for_ParameterIdentificationRun : ContextSpecification<ParameterIdentificationRun>
   {
      ...
      protected override void Context()
      {
         ...
         _parameterSelection1 = A.Fake<ParameterSelection>();
         A.CallTo(() => _parameterSelection1.Path).Returns("ParameterPath1");
         A.CallTo(() => _parameterSelection1.Simulation).Returns(_simulation);
         A.CallTo(() => _parameterSelection1.Parameter).Returns(_parameter1);
         A.CallTo(() => _parameterSelection1.Dimension).Returns(_parameter1.Dimension);

         _parameterSelection2 = A.Fake<ParameterSelection>();
         A.CallTo(() => _parameterSelection2.Path).Returns("ParameterPath2");
         A.CallTo(() => _parameterSelection2.Simulation).Returns(_simulation);
         A.CallTo(() => _parameterSelection2.Parameter).Returns(_parameter2);
         A.CallTo(() => _parameterSelection2.Dimension).Returns(_parameter2.Dimension);
   ...
   public class When_starting_the_parameter_identification_run_configured_with_a_parameter_identification : concern_for_ParameterIdentificationRun
   {
      ...
      protected override void Context()
      {
         ...
         A.CallTo(_algorithm).WithReturnType<OptimizationRunProperties>()
            .Invokes(x =>
            {
               _constraints = x.GetArgument<OptimizedParameterConstraint[]>(0);
            })
   ...
   public class When_the_parameter_identification_run_is_running_multiple_simulations : concern_for_ParameterIdentificationRun
   {
      ...
      protected override void Because()
      {
         ...
         _runResults.Each(x =>
         {
            _objectiveFunction(new[] {new OptimizedParameterValue("P", 100.0, 50d)});
         });
      }
