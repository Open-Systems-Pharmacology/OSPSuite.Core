...
public class When_updating_a_chart_from_another_chart : concern_for_CurveChart
{
   ...
   protected override void Context()
   {
      ...
      curve.SetyData(new DataColumn(), DimensionFactoryForSpecs.Factory);
      _other.AddCurve(curve);
      _other.FontAndSize.ChartHeight = 300;
      _other.ChartSettings.BackColor = Color.Black;
   }
   ...
   [Observation]
   public void should_update_curves_and_axes()
   {
      sut.Axes.Count.ShouldBeEqualTo(3);
      sut.Curves.Count.ShouldBeEqualTo(1);
      sut.Curves.Single().Name.ShouldBeEqualTo("curve");
   }

   [Observation]
   public void the_chart_settings_should_be_updated()
   {
      sut.ChartSettings.BackColor.ShouldBeEqualTo(Color.Black);
   }

   [Observation]
   public void the_font_and_size_settings_should_be_updated()
   {
      sut.FontAndSize.ChartHeight.ShouldBeEqualTo(300);
   }
}
