...
using Curve = OSPSuite.Core.Chart.Curve;
using CurveOptions = OSPSuite.Core.Chart.CurveOptions;
...
namespace OSPSuite.Core.Mappers
{
   ...
   public class When_mapping_a_snapshot_curve_to_curve_using_base_grid_as_x_axis : concern_for_CurveMapper
   {
      ...
      protected override async Task Context()
      {
         await base.Context();
         _snapshot = await sut.MapToSnapshot(_curve);
         _context = new SimulationAnalysisContext(new[] { _dataRepository }, new SnapshotContext(new TestProject(), SnapshotVersions.Current));
   ...
   public class When_mapping_a_snapshot_curve_to_curve_using_base_grid_as_x_axis_but_the_outputs_where_not_calculated : concern_for_CurveMapper
   {
      ...
      protected override async Task Context()
      {
         await base.Context();
         _snapshot = await sut.MapToSnapshot(_curve);
         _context = new SimulationAnalysisContext(null, new SnapshotContext(new TestProject(), SnapshotVersions.Current)) { RunSimulation = false };
         _newModelCurveOptions = new CurveOptions { Color = Color.Aqua };
   ...
   public class When_mapping_a_snapshot_curve_to_curve_using_another_column_as_x_axis : concern_for_CurveMapper
   {
      ...
      protected override async Task Context()
      {
         ...
         _context = new SimulationAnalysisContext(new[] { _dataRepository, _anotherRepository, }, new SnapshotContext(new TestProject(), SnapshotVersions.Current));
