...
namespace OSPSuite.Core.Mappers
{
   public abstract class concern_for_OutputIntervalMapper : ContextSpecificationAsync<OutputIntervalMapper>
   {
      ...
      protected OSPSuite.Core.Snapshots.OutputInterval _snapshot;
      ...
      protected override Task Context()
      {
         ...
         sut = new OSPSuite.Helpers.Snapshots.OutputIntervalMapper(_parameterMapper);
   ...
   public class When_mapping_output_interval_snapshot_to_output_interval : concern_for_OutputIntervalMapper
   {
      ...
      protected override async Task Because()
      {
         _newInterval = await sut.MapToModel(_snapshot, new SnapshotContext(new TestProject(), SnapshotVersions.Current));
      }

      [Observation]
      public void should_return_a_new_interval_with_updated_parameters()
      {
         A.CallTo(() => _parameterMapper.MapParameters(_snapshot.Parameters, _newInterval, Constants.OUTPUT_INTERVAL, A<SnapshotContext>._)).MustHaveHappened();
      }
   }
}
