...
using CurveChart = OSPSuite.Core.Chart.CurveChart;
...
namespace OSPSuite.Core.Mappers
{
   public abstract class concern_for_CurveChartMapper : ContextSpecificationAsync<CurveChartMapper>
   {
      ...
      protected override Task Context()
      {
         ...
         sut = new CurveChartMapper(_chartMapper, _axisMapper, _curveMapper, _idGenerator);
   ...
   public class When_mapping_a_curve_chart_snapshot_to_curve_chart : concern_for_CurveChartMapper
   {
      ...
      {
         await base.Context();
         _context = new SimulationAnalysisContext(null, new SnapshotContext(new TestProject(), SnapshotVersions.Current));
         _snapshot = await sut.MapToSnapshot(_curveChart);
