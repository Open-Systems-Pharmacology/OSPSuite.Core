...
namespace OSPSuite.Core.Mappers
{
   public abstract class concern_for_AxisMapper : ContextSpecificationAsync<AxisMapper>
   {
      ...
      protected override Task Context()
      {
         ...
         A.CallTo(() => _dimensionFactory.Has(_dimension.Name)).Returns(true);
   ...
   public class When_mapping_an_axis_snapshot_to_axis : concern_for_AxisMapper
   {
      ...
      protected override async Task Context()
      {
         ...
         A.CallTo(() => _dimensionFactory.MergedDimensionFor(A<DataColumn>.That.Matches(x => x.Dimension.Equals(_dimension)))).Returns(_optimalDimension);
      }

      protected override async Task Because()
      {
         _newAxis = await sut.MapToModel(_snapshot, new SnapshotContext(new TestProject(), SnapshotVersions.Current));
      }
