...
namespace OSPSuite.Core.Snapshots.Mappers
{
   public class OutputMappingMapper : SnapshotMapperBase<ModelOutputMapping, SnapshotOutputMapping, SnapshotContext>
   {
      ...
      private SimulationQuantitySelection outputSelectionFrom(string outputFullPath, SnapshotContext snapshotContext)
      {
         ...
         if (output == null)
         {
            _logger.AddWarning(Error.CouldNotFindOutputInSimulation(outputPath, simulationName));
            return null;
         }
      ...
      private IModelCoreSimulation simulationFromContext(string simulationName, SnapshotContext snapshotContext)
      {
         var simulation = snapshotContext.Project.All<IModelCoreSimulation>().FindByName(simulationName);
         if (simulation != null)
            return simulation;
