...
namespace OSPSuite.Core.Snapshots.Mappers
{
      ...
      where TSnapshot : new() where TSnapshotContext : SnapshotContext
   {
      ...
      public sealed override Task<TSnapshot> MapToSnapshot(TModel model) => Task.FromException<TSnapshot>(new ModelMapToSnapshotNotSupportedException<TSnapshot, TModelContext>());
   }
   ...
   {
      /// <summary>
      ///    Given a <paramref name="snapshot" /> object, returns the corresponding model using the
      ///    <paramref name="snapshotContext" />
      ///    as context
      /// </summary>
