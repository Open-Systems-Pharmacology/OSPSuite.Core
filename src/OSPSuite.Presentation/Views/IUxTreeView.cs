...
namespace OSPSuite.Presentation.Views
{
   public class NodeChangingEventArgs : EventArgs
   {
      ...
      public ITreeNode OldNode { get; private set; }
   ...
   public interface IUxTreeView 
   {
      ...
      /// <summary>
      /// Event occurs when a node is selected with hthe mouse
      /// </summary>
      event Action<MouseEventArgs, ITreeNode> NodeClick;
      ...
      /// <summary>
      /// Event occurs before the selected is changed, either programatically, with a keystroke or with a mouse click, thus allowing to cancel
      ...
      /// <summary>
      /// Add the node and all its subnode to the tree
      /// </summary>
      ...
      /// Destroy node by id
      /// </summary>
      /// <param name="id">id of the node to destrou</param>
      ...
      /// <summary>
      /// Collpase the <paramref name="node"/> if it is expanded or expand it if is collapsed
      /// </summary>
      /// <param name="node"></param>
      void ToggleExpandState(ITreeNode node);

      /// <summary>
      /// Selects the first node of the tree if none was selected otherwise reselect the former selected node (in order to raise events)
      /// </summary>
      ...
      /// <summary>
      /// returns the selected node if one node was selected, or the first node of a multi selection if more than one node is selected
      ...
      /// <summary>
      /// Show hint for a given cell. Typically called like this:
      /// You should drop a ToolTipController onto a form, set the TreeList's ToolTipController property and handle the ToolTipController.GetActiveObjectInfo event.
      /// </summary>
      /// <returns>A super tool tip containing the node tool tip if available that can be customized further, otherwise null if the tool tip is empty </returns>
      SuperToolTip ShowToolTip(ToolTipControllerGetActiveObjectInfoEventArgs e);
