...
namespace OSPSuite.Presentation.Presenters
{
   ...
   public abstract class LoadFromSnapshotPresenter<T, TProject, TSnapshotProject> : AbstractDisposablePresenter<ILoadFromSnapshotView, ILoadFromSnapshotPresenter>, ILoadFromSnapshotPresenter<T> where T : class, IObjectBase
   {
      protected readonly ISnapshotTask<TProject, TSnapshotProject> _snapshotTask;
      ...
      protected LoadFromSnapshotPresenter(
         ...
         IEventPublisher eventPublisher) : base(view)
      {
         ...
         _view.Caption = Captions.LoadObjectFromSnapshot(typeToLoad);
      ...
      {
         var message = Captions.LoadObjectFromSnapshot(typeToLoad);
      ...
      public async Task StartAsync()
      {
         try
         {
            ...
            _logger.AddInfo(Captions.LoadingSnapshot(_loadFromSnapshotDTO.SnapshotFile, typeToLoad));
            await Task.Run(performLoadAsync);
            _logger.AddInfo(Captions.SnapshotLoaded(typeToLoad));
         }
         catch (Exception e)
         {
