...
namespace OSPSuite.Presentation.Services.Importer
{
   public class Importer
   {
      ...
      public IList<ImportDataTable> ImportDataTables(ImportDataTable dataTableTemplate, string fileName, string sheetName, IList<ColumnMapping> mapping, Cache<string, Rectangle> rangesCache = null)
      {
         ...
         Action<DataTable> action = excelTable =>
         {
            tables = _importerTask.GetDataTables(fileName, sheetName, dataTableTemplate, mapping, excelTable);
      ...
      /// </summary>
      /// <remarks>This is needed to avoid errors later when the imported data is converted for chart display.
      /// When the charts have been introduced it has been decided to use data repositories.
      /// Now the component is not generic anymore.</remarks>
      ...
      /// <summary>
      /// This method retrieves a <see cref="DataSet"/> containing a <see cref="DataTable"/> for each excel sheet with first rows filled.
      /// </summary>
      /// <param name="fileName">Name of the excel file including full path.</param>
